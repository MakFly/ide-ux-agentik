  ...
  return (
    <div className="flex flex-col gap-0.5" aria-busy="true" aria-label="Loading branches">
      {BRANCH_LABEL_WIDTHS.map((labelW, i) => (
        <div key={i} className="mx-1.5 flex flex-col gap-1 rounded-md px-2 py-1.5">
...
export function WorktreesSkeleton() {
  ...
  return (
    <div className="flex flex-col gap-0.5 px-1.5" aria-busy="true" aria-label="Loading worktrees">
      {rows.map((r, i) => (
...
export function SessionsSkeleton() {
  const rows = [
    { title: "w-28", meta: "w-10", cmd: "w-40" },
    { title: "w-36", meta: "w-12", cmd: "w-32" },
    { title: "w-24", meta: "w-10", cmd: "w-44" },
  ];
  return (
    <div className="flex flex-col gap-0.5 px-1.5" aria-busy="true" aria-label="Loading sessions">
      {rows.map((r, i) => (
        <div key={i} className="flex w-full items-center gap-2 rounded-md px-2 py-1.5">
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <div className="flex items-center gap-1.5">
              ...
              <Skeleton className={`ml-auto h-2.5 ${r.meta}`} />
            </div>
            <Skeleton className={`h-2.5 ${r.cmd}`} />
          </div>
...
export function TasksSkeleton() {
  ...
  return (
    <div className="flex flex-col gap-0.5" aria-busy="true" aria-label="Loading tasks">
      {rows.map((r, i) => (
